...
The new features by [SIG-addons for TensorFlow 2.x](https://www.tensorflow.org/addons) make working with Keras even more exciting. Because ready to use additional functionalities help users to apply different techniques without any effort (e.g. custom losses, metrics, classes). But which functionalities exactly do SIG-addons provide us?
...
Indeed, this seems a bit challenging to understand but we can divide it into different sections:
...
1. The first thing we see are inputs. They are our plain *Input* texts. E.g.: "Very easy". Our input consists of 2 words.
2. Imagine that each word has 512 embeddings[@1301.3781]. Then we have 2 words (rows) with 512 columns (dimensions). Therefore, these are your *Input Embeddings*.
...
4. From the left of the plot, we see 4 layers: Multi-Head Attention mechanism, Layer Normalization, Feed Forward NN. What is the purpose of these layers? Before explanation, we can observe that in the middle, on the left side Encoder has so-called sub-encoders (Encoder_1, ..., Encoder_6). In fact, each of these encoders contains these 4 layers from Multi-head up to Layer normalization. From the right of the plot, we have 6 layers. This means that each Decoder has 6 of these layers.
5. Let's begin from the *Multi-head* layer:

$$\text{Attention}(Q,K,V)=\text{softmax}(\frac{QK^T}{\sqrt{d_k}})V $$
As we already mentioned above there are 3 type of matrices in Attention layer: Query, Key, and Value matrices where Q and K matrices are multiplied, then divided by the square root of Key matrix dimensions (in paper the number of Key dimensions are 64, so sqrt of 64 is 8). Then apply softmax activation in order to normalize the outputs. After that we have to multiply normalized value by Value matrix. 
...
Attention contains 3 Matrices, Heads consist of multiple Attentions. So, basically the purpose of Heads is to concentrate attention over the multiple Attention mechanisms and then concatenate all of Q, K, V matrices, and multiply with jointly created matrix:

$$W^O $$

6. The information from Attentions moves to next Layer - Feed Forward NN. But keep in mind that we never lose the dimensionality of our input vectors, 512. So, all the time inputs go into certain layer, is updated, and without losing dimension size is transferred to the next layer. Between Attention and Feed Forward NN, there are Normalisation Layers.

7. After delivering the output to the final Enocder_6, internal representation of vectors is transferred to the stack of decoders. What is on the right side of the diagram?
...
We use all 3 inputs from English and parallelly translate sentence, but which parallel operations? E.g.:
...
10. As final steps, the the output from Decoders stack are normalized by Softmax and Linear function (logits to probabilities)
...
- Define a function which will be used for generating a model with random defined parameters, such as the number of neurons, learning rates.
...
But we almost have forgotten to mention new callbacks! Now it is possible to stop training after a certain time and even get a progress bar while training a model. Just pass callbacks as usual into ```fit``` function:
...
- text ops
- rnn ops
- attention mechanisms, e.g: attention bahdanau, luong, monotonic and many more. [Neural machine translation](https://blogs.rstudio.com/ai/posts/2018-07-30-attention-layer/) was already explained. One can apply new methods from *tfaddons* to the same case or even directly find a new example [here](https://github.com/henry090/tfaddons/blob/master/vignettes/neural%20machine%20translation.R).

We encourage readers to experiment with these ops and share their perspectives on RStudio AI Blog!
